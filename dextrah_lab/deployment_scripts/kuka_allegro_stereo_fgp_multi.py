...
class DextrahFGP:
    def __init__(
        self, model_type, cfg_path, img_shape, num_proprio_obs,
    ...
    def step(self, proprio, left_image, right_image, hidden_states=None):
        if hidden_states is None and self.model_type == "rnn":
...
class DextrahFGPNode(Node):
    def __init__(self, node_name: str, model_type: str) -> None:
...
#            1)
    ...
    def init_fgp(self):
        ...
        config_yaml = None
        if self.model_type == "rnn":
            config_yaml = "rl_games_ppo_lstm_scratch_cnn_aux_stereo.yaml"
        else:
            config_yaml = "rl_games_ppo_transformer_stereo.yaml"
        ...
        if self.model_type == "rnn":
            ...
            #student_ckpt = "pretrained_ckpts/transformer/dextrah_student_110000.pth"
            student_ckpt = "pretrained_ckpts/transformer/dextrah_student_55000_iters.pth"
...
#        cv2.waitKey(1)
    ...
    def compute_actions(self, state, left_image, right_image, transmit=True, save_pth=False):
        ...
        if False: #transmit:
            action_dict = self.dextrah_fgp.step_cuda_graph(state, left_image, right_image, True)
        else:
            action_dict = self.dextrah_fgp.step_cuda_graph(state, left_image, right_image, False)
...
#            break
            ...
            with self._engage_fgp_lock:
                if self.engage_fgp is False:
                    ...
                    if self.dextrah_fgp.model_type == "rnn":
                        self.dextrah_fgp.reset_hidden_state()
                    else:
                        self.dextrah_fgp.reset()
...
if __name__ == "__main__":
    ...
    # Add a required argument with selectable values for model_type
    parser.add_argument(
        'model_type',
        choices=['rnn', 'transformer'],
        help="Specify the model type. Options are: 'rnn' or 'transformer'."
    )
    ...
    dextrah_fgp_node = DextrahFGPNode(node_name, model_type)
